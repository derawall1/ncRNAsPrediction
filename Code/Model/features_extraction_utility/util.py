...
def is_under_sequence_type(s, sequence_type):
    """Judge the string is within the scope of the sequence type or not.

    :param s: The string.
    :param sequence type: sequence_type.

    Return True or the error character.
    """
    for e in s:
        if e not in sequence_type:
            return e
...
def read_fasta_check_dna(f, sequence_type):
    ...
    for e in read_fasta_yield(f):
        res = is_under_sequence_type(e.seq, sequence_type)
        if res:
            seq_list.append(e)
        else:
            error_info = 'Sorry, sequence ' + str(e.no) \
                         + ' has character ' + str(res) + '.(The character must be ' + sequence_type + ').'
            sys.exit(error_info)

    return seq_list


def get_sequence_check_dna(f, sequence_type):
    """Read the fasta file.
...
def is_sequence_list(sequence_list, sequence_type):
    """Judge the sequence list is within the scope of sequence type and change the lowercase to capital."""
    count = 0
    new_sequence_list = []
    ...
    for e in sequence_list:
        e = e.upper()
        count += 1
        res = is_under_sequence_type(e, sequence_type)
        if res is not True:
...
def get_data(input_data, sequence_type, desc=False):
    ...
    if hasattr(input_data, 'read'):
        if desc is False:
            return get_sequence_check_dna(input_data, sequence_type)
        else:
            return read_fasta_check_dna(input_data, sequence_type)
    elif isinstance(input_data, list):
        input_data = is_sequence_list(input_data, sequence_type)
...
def convert_phyche_index_to_dict(phyche_index, sequence_type):
...
def make_kmer_list(k, sequence_type):
    try:
        return ["".join(e) for e in itertools.product(sequence_type, repeat=k)]
    except TypeError:
        print("TypeError: k must be an integer and larger than 0, sequence type must be a string.")
        raise TypeError
    except ValueError:
        print("TypeError: k must be an integer and larger than 0")
        raise ValueError
...
def normalize_index(phyche_index, sequence_type, is_convert_dict=False):
    ...
    if is_convert_dict is True:
        return convert_phyche_index_to_dict(normalize_phyche_value, sequence_type)

    print(normalize_phyche_value)
    return normalize_phyche_value


def read_k(sequence_type, _method, k):
    import const
...
def check_args(args, filename):
    ...
    if 'w' in args:
        if args.w < 0 or args.w > 1:
            ...
            return False
    if 'method' in args:
        if args.sequence_type == 'DNA' and args.method not in const.METHODS_DNA:
            ...
            return False
        elif args.sequence_type == 'RNA' and args.method not in const.METHODS_RNA:
            if filename == const.ACC_FILENAME:
            ...
            return False
        elif args.sequence_type == 'Protein' and args.method not in const.METHODS_PROTEIN:
            if filename == const.ACC_FILENAME:
...
if __name__ == '__main__':
    ...
    for e in (normalize_index(phyche_index, sequence_type='ACGT')):
        print(e)

    phyche_index_dict = normalize_index(phyche_index, sequence_type='ACGT', is_convert_dict=True)
    print(phyche_index_dict)
