...
query_seq = list(SeqIO.parse("NP_V2_lncRNA_seq_clean.fasta", "fasta"))
target_seq = list(SeqIO.parse("NP_V2_lncRNA_seq_clean.fasta", "fasta"))
...
def calculate_similarity(query_seq,target_seq):
    print("started task...")
    with open("/scratch1/Mushtaq/LncRNA_Similarity/LncRNA-LncRNA_Similarity.csv", "wb") as csvFile:
        # csv file header
                ...
                for seq_t in target_seq:
                    if seq_q.seq != seq_t.seq:
                        ...
                        try:
                            ...
                            #print(seq_len)
                            Identity = round_down(float(matches)/seq_len * 100,2)
                            #print(Identity)

                            #Identity =str(round_down(matches/(matches + mismatches) * 100,2)) +'%' 
                            # calculating the similarity in %
                            Similarity=round_down(float(matches)/seq_len * 100,2)
